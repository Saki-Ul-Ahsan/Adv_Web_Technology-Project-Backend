...
import { SupportDTO, agentDTO, bookingsInfo, tourPackagesInfo } from './agent.dto'; 
import { PasswordUtil } from 'src/Utils/bcrytp';
...
export class agentService {
  ...
  getSupportById(): SupportEntity[]| Promise<SupportEntity[]>{
    throw new Error('Method not implemented.');
  }

  getBookings(): agentbookingsEntity[]| Promise<agentbookingsEntity[]>{
    throw new Error('Method not implemented.');
  }
...
for(const agent of agents){
  const emailOptions = {
    ...
    text: `Dear ${agent.name},\n\nA new tour package added. Check it out. Tour ID:${res.tour_id}, Creator_id:${res.creator_id}, from:${res.from} and  destination:${res.destination}`
  };
...
async findAgentByName(name: string): Promise<agentEntity | undefined> {
  return this.agentRepo.findOne({ where: { name: Like(`%${name}%`) } });
}
...
async getAllBookings(): Promise<agentbookingsEntity[]> {
  return await this.agentBookingsRepo.find();
}
...
async login(credentials: agentDTO): Promise<boolean> {
  ...
  if (!agent) {
    throw new UnauthorizedException(`Agent with email: ${credentials.email} not found`);
  }
