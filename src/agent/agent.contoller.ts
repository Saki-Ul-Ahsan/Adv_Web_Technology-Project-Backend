import { Body, Controller, Get, Query, Param, Post, UsePipes, ValidationPipe, UseInterceptors, UploadedFile, Res, Put, Delete, Session, HttpException, HttpStatus, UseGuards, UnauthorizedException, Req, Patch, ParseIntPipe } from '@nestjs/common';
...
async searchAgentByName(@Param('name') name: string): Promise<agentEntity | undefined> {
  return this.agentService.findAgentByName(name);
}
...
@Get('getBookings')
getAllBookings(): Promise<agentbookingsEntity[]>{
  return this.agentService.getAllBookings()
}
...
async login(@Body() credentials: agentDTO, @Session() session) {
  try {
    ...
    throw new HttpException(`Agent with email: ${credentials.email} not found`, HttpStatus.UNAUTHORIZED)
  }
}
